    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():  # don't track gradients during validation
            ...
            if (
                ...
                torch.save(model, "models/best_classifier.pt")  # checkpoint model
...
def train_regression():
    ...
    epochs = 1000
    patience = 50
    print_every = 1  # print loss every x epochs
    ...
    # Evaluate model
    print("Evaluating model.")
    test_rmse = torch.sqrt(torch.nn.MSELoss()(model(data)[dataset.mask_te], data.y[dataset.mask_te]))  # Only test nodes
